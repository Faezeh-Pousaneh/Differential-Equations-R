...
## =============================================================================
# J is Jstar which is in unit of kBTc, and Psistar is in unit of e/kBTc, h is h over kBtc, potential is in 
#units of kBTc.
...
while ( s<0.05)  {
	...
	for(z in seq(5,100,5)) {
		...
		Sol1[,ncol(Sol)+3]=  0.5*J* ( -6*(Sol1[,2])*(Sol1[,2])+(Sol1[,3])*(Sol1[,3]))   -
							 ...
							 0+
							 T*(0.5*(Sol1[,2])*(Sol1[,2])    + (0.5/p)* ((((2*A-(-A*(tanh(Sol1[,4]/b))^2+A)^0.5)*(1+Sol1[,2]))/(4*A+(tanh(Sol1[,4]/b))^2-1))   * tanh(Sol1[,4]/b))^2 
							 - (0.5/p)* Sol1[,2]*((((2*A-(-A*(tanh(Sol1[,4]/b))^2+A)^0.5)*(1+Sol1[,2]))/(4*A+(tanh(Sol1[,4]/b))^2-1))   * tanh(Sol1[,4]/b))^2)
...
matplot((Sol5[,1]), (Sol5[,2]-Sol5[((100-2)/5),2]), type = "l", lty = 1,  col = c("red"),lwd=2,font=5, xlab = "z",ylab="Potential")
	#for (i in 0:ncol(Sol1))
		write.table(Sol5 ,file="Fig11.Exp.txt")
